...
public class Controlador extends HttpServlet {
    ...
    private Destino handleKeep(HttpServletRequest request, HttpServletResponse response,
            String op, String accion, String origen) {
 //http://192.168.208.110:8080/Keep2/go?tabla=keep&op=read&origen=android&accion=&login=pepe
        switch (op) {
            case "create":
                if (origen.equals("android")) {
                ...
                if (origen.equals("web")) {
                    ...
                    for (Keep k : keeps) {
                        if (k.getIdAndroid() == min) {
                            min++;
                        } else{
                            continue;
                        }
                    }
                    Keep k = new Keep(null, min, request.getParameter("contenido"), null, "inestable");
                    JSONObject obj = GestorKeep.addKeep(k, request.getParameter("login"));
            ...
            case "update":
                if(origen.equals("android")){
                    GestorKeep.updateKeep(Integer.parseInt(request.getParameter("id")), request.getParameter("contenido"), 
                            Integer.parseInt(request.getParameter("idAndroid")), request.getParameter("estado"));
